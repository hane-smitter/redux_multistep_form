...
const Pages = () => {
  ...
  const reduxState = useSelector((state) => state.val);
            ...
            <FormStep>
              <Box paddingBottom={2}>
                <Field
                  ...
                  component={TextField}
                  label="condition"
                  value={reduxState.condition}
                  onChange={(e) =>
                    dispatch({type: 'SET_COND', payload: e.target.value})
                  }
                >
...
function FormikStepper({ children, ...props }) {
  ...
  const globValues = useSelector((state) => state.val);
